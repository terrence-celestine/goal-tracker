  ...
  if (req.method === 'POST') {
    ...
    if (action === 'register') {
      ...
      try {
        ...
        const token = jwt.sign({ userId: newUser.id, email, displayName: username }, JWT_SECRET, { expiresIn: '7d' })
        setTokenCookie(res, token);

        return res.status(201).json({ userId: newUser.id, email, displayName: username });
